...
6. Какие итераторы бывают?
7. Как в произвольное место в векторе вставить число?
8. Назовите все контейнеры STL?
9. Чем работа list отличается от vector?
10. Назовите методы контейнеров STL?
11. Чем отличается префисный инкремент от постфиксного?
12. Чем отличается NULL от nullptr?
13. Что такое массив указателей?
14. Что такое указатель? Ссылка?
15. Как создать и очистить двухмерный массив?
16. Как реализовать добавление и удаление в динамической массиве?
17. Как узнать код символа и сам символ?
18. Назовите методы c-style строк?
19. Что такое оператор goto?
20. Почему цикл foreach не работает с динамическим массивами?
21. Что такое int argc и char *argv[] в функции main?
22. Как объединить строки в стиле C?
23. Что такое указатель на функцию?
24. Чем опасно использовать директиву препроцессора define?
25. Когда наиболее лучше всего использовать директивы ifdef и endif?
26. Что такое обработчики cath all?
27. Как объявить лямду? Что такое и как сделать захват переменных? Что такео универсальные лямды? Как передать лямду?
28. Как происходит работа с потоками на примере функции подсчета факториала 3х чисел? (A)
29. Зачем нужен виртуальный деструктор?
30. Как защитить объект от копирования?
31. В чем разница между struct и class?
32. Сколько в памяти занимает произвольная структура?
33. В чем отличие vector от deque?
34. Для чего нужен аллокатор?
35. В чём суть множественного наследования? Какие проблемы могут возникнуть при его использовании? Как их преодолеть?
36. В чем основное различие между деструктором и оператором delete?
37. В чем разница между массивом и списком?
38. Что делает функция Friend в C ++?
39. Что такое виртуальные функции?
40. В чем разница между мьютексом и семафором?
41. Как срабатывают конструкторы и деструкторы при наследовании в с++
42. Что такое string::npos?
43. Для чего нужен спецификатор final?
...
4. "Куча" (heap) и "стек вызовов" (call stack) - это две разные области памяти. Стек вызовов используется для хранения информации о функциях, которые в данный момент вызываются в вашей программе. Куча используется для динамического выделения памяти во время выполнения программы. Это место, где вы можете создавать объекты и структуры данных переменного размера, и они остаются в памяти до явного освобождения.
5. -
6. ``vec.insert(vec.begin() + 2, 10); // вставляем 10 на позицию 2``
7. vector, list, deque, array, forward_list, set, map, multiset, multimap, unordered_set, unordered_map, unordered_multiset, unordered_multimap, stack, queue, priority_queue
...
24. Такие директивы лучше всего использовать тогда, когда у на есть какая то отладочная информация (cout что то например), и мы не хотим их удалить или комментить. Мы можем их обернуть в дерективы и управлять, опять выводить или нет. Также есть ещё #else
...
40. ```
    ...
    // A, B, C, ~C, ~B, ~A
    ```
41. Это константа в C++, которая используется для обозначения "не найденного" значения при работе со строками. Если равен npos, то не найдено.
42. С помощью спецификатора final мы можем запретить наследование: class Person final {};
